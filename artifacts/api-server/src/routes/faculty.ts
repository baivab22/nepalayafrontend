...
import { FacultyModel, facultySchemaZ } from "@workspace/db/schema";
...
const uploadDir = path.resolve(process.cwd(), "artifacts/api-server/uploads/faculty-photo");
...
router.post("/faculty/upload-photo", upload.single("photo"), (req, res) => {
  ...
  // Build absolute URL
  // Always use http://localhost:8000 for the returned URL as requested
  const url = `http://localhost:8000/api/faculty/photo/${file.filename}`;
