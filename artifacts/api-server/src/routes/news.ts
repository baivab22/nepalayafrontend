...
import { NewsModel, newsSchemaZ } from "@workspace/db/schema";
...
const newsUploadDir = path.resolve(
  process.cwd(),
  "artifacts/api-server/uploads/news-images",
);
...
router.post("/news/upload-image", newsUpload.single("image"), (req, res) => {
  ...
  // Always return absolute URL as consumed by the frontend
  const url = `http://localhost:8000/api/news/image/${file.filename}`;
